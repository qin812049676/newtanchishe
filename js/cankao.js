$(function(){
	...
	var huashe  = function(){
		$.each(snake,function(index,value){
			$('#'+ value.x + '_'+ value.y).css({background:'#183c03'});
		})
	}
	huashe();
	...
	var move = function () {
		...
		if(newTou.x == food.x && newTou.y == food.y){
			food = dropFood();
			num+=15;
			$('#shu').find('span').text(num);
			
		}else{
			var weiba = snake.shift();
			delete data[weiba.x+'_'+weiba.y];
			$('#'+weiba.x+'_'+weiba.y).css({background:''});
		}
		snake.push(newTou)
		data[newTou.x + '_' + newTou.y] = true;
		$('#'+newTou.x + '_' + newTou.y).css({background:'#183c03'})
	}	
